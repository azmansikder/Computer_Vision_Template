# ==============================================================================
# 🏆 ULTIMATE FINAL TEMPLATE v5.0 — PRODUCTION & RESEARCH READY
...
# ------------------------------------------------------------------------------
# v5.0 NEW vs v4.0:
#   ✅ NEW 1: Medical Preprocessing Toggle (Gaussian Blur + CLAHE for Contrast)
#   ✅ NEW 2: Qualitative Visual Analysis (Plots True vs Pred with Confidence)
# v4.0 NEW vs v3.0:
...
import cv2   # ✅ NEW: Medical Image Preprocessing এর জন্য OpenCV
import numpy as np
import pandas as pd
...
class CFG:
    ...
    # ✅ NEW: Medical Dataset Toggle
    # True করলে Gaussian Blur + CLAHE apply হবে (X-Ray, Retina, MRI এর জন্য দারুণ)
    # সাধারণ ইমেজের (Dog, Cat, Plant) জন্য False রাখো
...
if CFG.data_format == 'csv':
    ...
    # ────────────────────────────────────────────────────────
    # ✅ Universal One-Hot → Single Label Auto-Converter
...
# ==========================================
# 3. DATA GENERATORS & PREPROCESSING
# ==========================================
# ⚠️ CRITICAL: rescale=1./255 দেওয়া যাবেই না
#    Model-এ dynamic preprocessing আছে (Section 6)

# ✅ NEW: Medical Preprocessing Logic
def custom_preprocessing(img):
    """
    CFG.use_medical_preprocessing = True হলে এটি কাজ করবে।
    Medical Image-এর নয়েজ কমায় এবং CLAHE দিয়ে কন্ট্রাস্ট বাড়ায়।
    """
    if not getattr(CFG, 'use_medical_preprocessing', False):
        return img
    
    # 1. Keras float32 (0-255) পাঠায়, OpenCV uint8 চায়
    img_uint = np.uint8(img)
    
    # 2. Gaussian Blur (Reduce High-Frequency Noise)
    img_blur = cv2.GaussianBlur(img_uint, (3, 3), 0)
    
    # 3. CLAHE (Contrast Limited Adaptive Histogram Equalization)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    
    # CLAHE শুধু Grayscale বা LAB কালার স্পেসে ভালো কাজ করে
    lab = cv2.cvtColor(img_blur, cv2.COLOR_RGB2LAB)
    lab[:,:,0] = clahe.apply(lab[:,:,0])
    img_clahe = cv2.cvtColor(lab, cv2.COLOR_LAB2RGB)
    
    # Keras আবার float32 আশা করে
    return np.float32(img_clahe)


train_datagen = ImageDataGenerator(
    preprocessing_function=custom_preprocessing,  # ✅ যুক্ত করা হলো
    rotation_range=15,
...
val_datagen = ImageDataGenerator(preprocessing_function=custom_preprocessing) # ✅ যুক্ত করা হলো
...
def run_pipeline(model_func, model_name):
    ...
    # ✅ NEW: Qualitative Evaluation Plot কল করা হলো
    _plot_qualitative_results(model, val_df, model_name)
...
# ✅ NEW: Qualitative Evaluation Plot Function
def _plot_qualitative_results(model, val_df_eval, name, num_images=8):
    """
    Qualitative Analysis: মডেল আসল ছবির সাপেক্ষে কী প্রেডিক্ট করলো তা দেখানো।
    এটি রিসার্চ পেপার এবং প্রেজেন্টেশনের জন্য খুব গুরুত্বপূর্ণ।
    """
    try:
        print(f"\n🔍 Generating Qualitative Visuals for {name}...")
        # Shuffle=True দিয়ে রান্ডম ব্যাচ নেওয়া হচ্ছে
        plot_gen = make_gen(val_datagen, val_df_eval, shuffle=True)
        x_batch, y_batch = next(plot_gen)
        
        preds = model.predict(x_batch[:num_images], verbose=0)
        ...
        for i in range(min(num_images, len(x_batch))):
            ...
            img = x_batch[i]
            # যদি image [0, 1] রেঞ্জে থাকে, তবে প্লট করার জন্য 255 দিয়ে গুণ করে নিতে হবে
            if img.max() <= 1.0: 
                img = img * 255.0
            plt.imshow(img.astype('uint8'))
            
            if CFG.task_type == 'multiclass':
                true_label = index_to_class[np.argmax(y_batch[i])]
                pred_label = index_to_class[np.argmax(preds[i])]
        ...
        plt.tight_layout()
        plt.savefig(f'{name}_qualitative.png', dpi=150)
        plt.show()
    except Exception as e:
        print(f"  ⚠️ Qualitative Plot Skipped: {e}")
